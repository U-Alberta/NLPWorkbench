...
from . import evaluator, features, global_features, configs as config, uri_instance as UriInstance
...
class Instance:
  ...
  def featurize(self):
    ...
    if MODEL == "ours" or MODEL == "bert":
      ...
      global_features_to_use = np.concatenate(gf)
  ...
  def facts(self):
    ...
    for predicate_id in self.positive_labels():
        score = self.scores[predicate_id]
        fact = {
          "subject": self.subject_entity.text,
          "predicate": PROPERTY_NAMES[predicate_id],
          "object": self.object_entity.text,
          "score": float(score),
          "sents": [self.subject_entity.sent.text, self.object_entity.sent.text],
        }
        _facts.append(fact)
    return _facts