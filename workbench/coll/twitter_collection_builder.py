...
def build_tweet_collection(
    ...
    for tweet_id in tweet_ids:
        ...
        if author_username[0] not in all_usernames:
            ...
            user_imports = build_username_collection(
                neo_connection,
                tweepy_client,
                es_client,
                author_username,
        ...
        # Merge into elastic search (handles both update or creation)
        merge_doc(
            client=es_client, index_name=es_index_name, doc_id=doc["id"], doc_data=doc
        )
        imported_docs += 1
    return imported_docs


def build_username_collection(
    neo_connection: NeoConnection,
    tweepy_client: Client,
    es_client: Elasticsearch,
    usernames,
