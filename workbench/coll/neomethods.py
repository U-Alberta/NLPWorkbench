...
def get_users(neo_connection: NeoConnection):
    ...
    """
    return neo_connection.exec_query(
        "MATCH (n:User) RETURN (n.username) AS username", get_result=True
    )
...
def get_tweets(neo_connection: NeoConnection):
    ...
    """
    return neo_connection.exec_query(
        "MATCH (n:Tweet) RETURN (n.id) AS tweetId", get_result=True
    )
...
def tweet_exists(neo_connection: NeoConnection, tweet_id):
    ...
    """
    if len(
        neo_connection.exec_query(
            "MATCH (n:Tweet) WHERE n.id=$nid RETURN (n)", get_result=True, nid=tweet_id
        )
    ):
        return True
    return False
...
def user_exists(neo_connection: NeoConnection, user_id):
    ...
    if len(
        neo_connection.exec_query(
            "MATCH (n:User) WHERE n.id=$nid RETURN (n)", get_result=True, nid=user_id
        )
    ):
        return True
    return False
...
def create_tweet(
    neo_connection: NeoConnection, tweet_data, relations: list, es_index_name
):
    """
                    ...
                    for likes in Paginator(
                        ...
                        if likes.data:
                            for liking_user in likes.data:
                                create_tweet_relation(
                                    neo_connection, liking_user, tweet["id"], "LIKED"
                                )
                    ...
                    for retweets in Paginator(
                        ...
                        if retweets.data:
                            for retweeter in retweets.data:
                                create_tweet_relation(
                                    neo_connection, retweeter, tweet["id"], "RETWEETED"
                                )
...
def create_author(neo_connection: NeoConnection, user_id, tweet_id):
    ...
    """
    if not user_exists(neo_connection, user_id) or not tweet_exists(
        neo_connection, tweet_id
    ):
...
def create_tweet_relation(neo_connection: NeoConnection, user, tweet_id, relation):
    ...
    if user_exists(neo_connection, user["id"]):
        ...
        neo_connection.get_session().run(
            transaction, username=username, id=tweet_id, relation=relation
        )
    else:
        ...
        neo_connection.get_session().run(
            transaction, username=username, id=user["id"], name=name, tweet_id=tweet_id
        )
...
def create_follower(neo_connection: NeoConnection, follower, user_id):
    ...
    if user_exists(
        neo_connection, follower["id"]
    ):  # Follower exists as a User within graph
        neo_connection.get_session().run(
...
def run_pagerank(neo_connection: NeoConnection, name, entities: list, rel, attribute):
    ...
    # Clean up
    neo_connection.get_session().run(f"""CALL gds.graph.drop('{name}')""")
...
def get_subgraph_query(neo_connection: NeoConnection, username):
    ...
    if username not in all_usernames:
        input(
            "[-] Error: there is no existing user with the username '{}'. Press enter to continue...".format(
                username
            )
        )
        return None
