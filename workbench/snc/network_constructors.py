    ...
    for tweet_id in tweet_ids:
        ...
        if len(tweet_query):  # Tweet already exists within neo4j
            ...
            session.run("MERGE (u:User {username: $username})"
                        ...
                        "CREATE (u)-[:AUTHORED]->(t)",
                        username = username, id = tweet_id, author_id = tweet.data['author_id'], created_at = tweet.data['created_at'],
                        likes = tweet.data['public_metrics']['like_count'], retweets = tweet.data['public_metrics']['retweet_count'], 
                        replies = tweet.data['public_metrics']['reply_count'], text = text
            )
            ...
            if "entities" in tweet.data:
                if "mentions" in tweet.data["entities"]:
                    for user in tweet.data["entities"]["mentions"]:
                        session.run("MERGE (u:User {username: $username, id: $id}"
                                    "MERGE (t:Tweet {id: $tweet_id})"
                                    "MERGE (t)-[:MENTIONS]->(u)",
                                    username = user['username'], id = user['id'], tweet_id = tweet_id, 
                        )
                ...
                if "annotations" in tweet.data["entities"]:
                    for annotation in tweet.data["entities"]["annotations"]:
                        ...
                        session.run(transaction, 
                                    desc = annotation['normalized_text'], id = tweet_id, probability = annotation['probability']
                        )
    ...
    for username in usernames:
        ...
        if len(user_query):  # User already exists in neo4j
            ...
            session.run("CREATE (:User{\
                        ...
                        username = username, id = user.data['id'], desc = user.data['description'], name = user.data['name'],
                        followers = user.data['public_metrics']['followers_count'], following = user.data['public_metrics']['following_count'],
                        tweet_count = user.data['public_metrics']['tweet_count'], created_on = str(user.data['created_at'])[0:10]
            )
                    ...
                    for tweet in recent_tweets.data:
                        ...
                        session.run("MERGE (u:User{username: $username})"
                                    "MERGE (t:Tweet{\
                                        ...
                                        text: $text})"
                                    "CREATE (u)-[:AUTHORED]->(t)",
                                    username = username, tweet_id = tweet_id, author_id = tweet['author_id'], created_at = tweet['created_at'],
                                    likes = tweet['public_metrics']['like_count'], retweets = tweet['public_metrics']['retweet_count'],
                                    replies = tweet['public_metrics']['reply_count'], text = text
                        )
                        # Push data regarding mentioned users/people/places
