...
class AMRData(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        train_path = (
            self.config.data_files["train"]
            if "train" in self.config.data_files
            else None
        )
        dev_path = (
            self.config.data_files["validation"]
            if "validation" in self.config.data_files
            else None
        )
        test_path = (
            self.config.data_files["test"] if "test" in self.config.data_files else None
        )

        train_generator = datasets.SplitGenerator(
            name=datasets.Split.TRAIN, gen_kwargs={"filepath": train_path}
        )
        dev_generator = datasets.SplitGenerator(
