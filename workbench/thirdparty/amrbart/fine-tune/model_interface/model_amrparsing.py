...
class AMRParsingModelModule(pl.LightningModule):
    ...
    def __init__(self, tokenizer, args, bart_model=None):
        super().__init__()
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        if "preds" in outputs[0]:
            ...
            import logging
            logging.info(f"Saving validation predictions to {output_test_predictions_file}")
            # write predictions and targets for later rouge evaluation.
            with open(output_test_predictions_file, "w") as p_writer:
                logging.info("we are writing something....")
                p_writer.write("\n\n".join(pieces))
            try:
