...
def main(argv):
    ...
    sys.stderr.write(
        "Splitting {} lines ({} documents)\n".format(
            corpus_size, total_docs + (1 if leftover else 0)
        )
    )
    sys.stderr.write(
        "Train: {} ({})\n".format(
            (train_docs * doc_size) + leftover, train_docs + (1 if leftover else 0)
        )
    )
    sys.stderr.write(
        "Dev: {} x {} ({})\n".format(
    ...
    dev_out = [
        open("{}.dev.{}".format(out_prefix, i + 1), "w") for i in range(dev_sets)
    ]
    i = 0
    ...
    for _ in range(docs_per_dev_set):
        ...
        for _ in range(train_batch_size):
            for _ in range(doc_size):
                i += 1
                train_out.write(
                    "{} ||| {}".format(i, inp.readline()) if lc else inp.readline()
                )
