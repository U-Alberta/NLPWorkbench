...
TOKENIZER = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "tokenize-anything.sh"
)
...
def main(argv):
    if len(argv[1:]) < 1:
        sys.stderr.write(
            "Parallelize text normalization with multiple instances of tokenize-anything.sh\n\n"
        )
        sys.stderr.write(
            "Usage: {} in-file [jobs [temp-dir]] >out-file\n".format(argv[0])
        )
        sys.exit(2)
