...
def save_model(model, args):
    ...
    model_to_save = (
        model.bert_model.module
        if hasattr(model.bert_model, "module")
        else model.bert_model
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_epoch", type=int, default=100, help="number of the training epochs"
    )
    parser.add_argument(
        "--print_loss_step",
        type=int,
    ...
    parser.add_argument(
        "--do_eval", action="store_true", help="whether to run evaluation"
    )
    parser.add_argument(
