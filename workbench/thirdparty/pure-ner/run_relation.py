...
def main(args, model_manager=None):
    ...
    # train set
    if args.do_train:
        train_dataset, train_examples, train_nrel = generate_relation_data(
            args.train_file, use_gold=True, context_window=args.context_window
        )
    # dev set
    ...
    if model_manager is not None:
        tokenizer = model_manager.get_preloaded_model("rel-tokenizer")
    ...
    if model_manager is not None:
        model_manager.save_preloaded_model("rel-tokenizer", tokenizer)
    ...
    if args.do_train:
        ...
        all_sub_idx = torch.tensor(
            [f.sub_idx for f in train_features], dtype=torch.long
        )
        all_obj_idx = torch.tensor(
            [f.obj_idx for f in train_features], dtype=torch.long
        )
        ...
        model = None
        if model_manager is not None:
            model = model_manager.get_preloaded_model("rel")
        if model is None:
            ...
            if model_manager is not None:
                model_manager.save_preloaded_model("rel", model)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_batches):
                ...
                loss = model(
                    input_ids, segment_ids, input_mask, label_ids, sub_idx, obj_idx
                )
                if n_gpu > 1:
                    loss = loss.mean()
    ...
    if args.do_eval:
        ...
        if args.eval_test:
            ...
            all_sub_idx = torch.tensor(
                [f.sub_idx for f in eval_features], dtype=torch.long
            )
            all_obj_idx = torch.tensor(
                [f.obj_idx for f in eval_features], dtype=torch.long
            )
            eval_data = TensorDataset(
                all_input_ids,
                all_input_mask,
...
def call_from_external(cmd_args=None, model_manager=None):
    global args
