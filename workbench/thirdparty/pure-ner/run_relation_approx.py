        ...
        for i, sent in enumerate(doc):
            ...
            for x in range(len(sent_ner)):
                for y in range(len(sent_ner)):
                    ...
                    if (len(tokens) + 4 > max_seq_length) or (
                        not (batch_computation) and len(tokens) > num_tokens
                    ):
                    ...
                    position_ids = position_ids + [
                        token_start[sent_start + sub.span.start_sent],
                        token_end[sent_start + sub.span.end_sent],
                        token_start[sent_start + obj.span.start_sent],
                        token_end[sent_start + obj.span.end_sent],
                    ]
            ...
            if num_shown_examples < 20:
                ...
                logger.info(
                    "input_ids: %s" % " ".join([str(x) for x in features[-1].input_ids])
                )
                logger.info(
                    "position_ids: %s"
                    % " ".join([str(x) for x in features[-1].position_ids])
                )
                logger.info(
                    "input_mask: %s"
                    % " ".join([str(x) for x in features[-1].original_input_mask])
                )
                logger.info(
                    "labels: %s" % " ".join([str(x) for x in features[-1].labels])
                )
                logger.info(
                    "sub_obj_ids: %s"
                    % " ".join(
                        ["(%d, %d)" % (x[0], x[1]) for x in features[-1].sub_obj_ids]
                    )
                )
                logger.info(
                    "sub_obj_masks: %s"
                    % " ".join([str(x) for x in features[-1].sub_obj_masks])
                )
                logger.info(
                    "sub_obj_spans: %s"
                    % " ".join([str(x) for x in features[-1].meta["sub_obj_pairs"]])
                )
...
def evaluate(model, device, eval_dataloader, e2e_ngold=None, verbose=True):
    ...
    if verbose:
        logger.info(
            "***** Eval results (used time: %.3f s) *****" % (time.time() - c_time)
        )
...
def main(args):
    if "albert" in args.model:
        raise ValueError(
            "ALBERT approximation model is not supported by the current implementation, as Huggingface's Transformers ALBERT doesn't support an attention mask with a shape of [batch_size, from_seq_length, to_seq_length]."
        )
    ...
    if args.do_eval and (args.do_train or not (args.eval_test)):
        ...
        all_position_ids = torch.tensor(
            [f.position_ids for f in eval_features], dtype=torch.long
        )
        ...
        all_sub_obj_ids = torch.tensor(
            [f.sub_obj_ids for f in eval_features], dtype=torch.long
        )
        all_sub_obj_masks = torch.tensor(
            [f.sub_obj_masks for f in eval_features], dtype=torch.long
        )
    ...
    if args.do_train:
        ...
        all_position_ids = torch.tensor(
            [f.position_ids for f in train_features], dtype=torch.long
        )
        all_input_mask = torch.tensor(
        ...
        all_sub_obj_ids = torch.tensor(
            [f.sub_obj_ids for f in train_features], dtype=torch.long
        )
        all_sub_obj_masks = torch.tensor(
            [f.sub_obj_masks for f in train_features], dtype=torch.long
        )
        ...
        optimizer = AdamW(
            optimizer_grouped_parameters, lr=lr, correct_bias=not (args.bertadam)
        )
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_batches):
                ...
                if (step + 1) % eval_step == 0:
                    ...
                    save_model = False
                    if args.do_eval:
                        preds, result = evaluate(
                            model, device, eval_dataloader, e2e_ngold=eval_nrel
                        )
                        model.train()
    ...
    if args.do_eval:
        if args.eval_test:
            ...
            eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size)
        model = RelationModel.from_pretrained(
            args.output_dir, num_rel_labels=num_labels
        )
        model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_file", default=None, type=str, help="The path of the training data."
    )
    parser.add_argument(
        "--train_mode",
        type=str,
    ...
    parser.add_argument(
        "--eval_test",
        action="store_true",
        help="Whether to evaluate on final test set.",
    )
    parser.add_argument(
        "--eval_with_gold",
        action="store_true",
