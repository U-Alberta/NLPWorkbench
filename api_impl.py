...
def extract_relations():
    ...
    texts = [news["title"] + "\n" + news["content"]]
    output = re_celery.signature("relation.run_batch", args=[texts]).delay().get()
    for fact in output:
        fact["sents"] = list(set(fact["sents"]))
    return output


def build_sn(data, data_type, time_frame):
    snc_celery = create_celery("api_impl", "snc")
    # located at snc/snc.py
    # TODO: change this to async call
    status = snc_celery.signature("snc.run_snc", args=(data, data_type, time_frame)).delay().get()
    return status