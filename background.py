...
celery = create_celery("background")
relation_re = create_celery("bg_relation_re", "relation")
...
def precompute_re(doc_id):
    ...
    with allow_join_result():
        ...
        # TODO: test this
        re_output = re_celery.signature("relation.run_batch", args=[doc["title"] + "\n" + doc["content"]]).apply_async(priority=0).get()
...
if __name__ == '__main__':
    celery.start(argv=["-A", "background", "worker", "-l", "INFO", "--concurrency=5", "-Q", "background", "-n", "background-worker@%n"])