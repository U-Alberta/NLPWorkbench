...
class BertForEntity(BertPreTrainedModel):
    ...
    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        sequence_output = self.bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask).last_hidden_state
...
class EntityModel():
    ...
    def __init__(self, args, num_ner_labels):
        ...
        if args.bert_model_dir is not None:
            bert_model_name = args.bert_model_dir
    ...
    def _get_input_tensors_batch(self, samples_list, training=True):
        ...
        if len(tokens_tensor_list) == 0:
            raise AssertionError("all samples are filtered out")
