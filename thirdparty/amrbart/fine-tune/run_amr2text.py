...
def main(args, model_manager = None):
    ...
    if model_manager is not None:
        amr_tokenizer = model_manager.get_preloaded_model("amr2text_tokenizer")
    if amr_tokenizer is None:
        ...
        model_manager.save_preloaded_model("amr2text_tokenizer", amr_tokenizer)
    ...
    bart_model = None
    if model_manager is not None:
        bart_model = model_manager.get_preloaded_model("amr2text")
    if bart_model is None:
        model = AMR2TextModelModule(amr_tokenizer, args)
        if model_manager is not None:
            model_manager.save_preloaded_model("amr2text", model.model)
    else:
        model = AMR2TextModelModule(amr_tokenizer, args, bart_model=bart_model)
