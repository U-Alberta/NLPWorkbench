...
#      (This is a very unlikely use case).
    ...
    if running_as_root && ! secure_mode_enabled; then
        ...
        if ! is_writable "${mountFolder}"; then
            ...
            echo "Consider unsetting SECURE_FILE_PERMISSIONS environment variable, to enable docker to write to ${mountFolder}."
...
function load_plugin_from_location
{
  ...
  if ! is_readable "${_destination}"; then
    echo "Plugin at '${_destination}' is not readable"
    exit 1
  fi
}
...
function load_plugin_from_github
{
  ...
  if [[ -z "${_plugin_jar_url}" ]]; then
    echo "Error: No jar URL found for version '${_neo4j_version}' in versions.json from '${_versions_json_url}'"
    exit 1
  fi
    ...
    if [ "${cmd}" == "neo4j" ]; then
        if [ "${_neo4j_auth:-}" == "none" ]; then
            ...
            if [ "${admin_user}" != "neo4j" ]; then
                echo "Invalid admin username, it must be neo4j"
                exit 1
            fi
            ...
            echo "Invalid value for NEO4J_AUTH: '${_neo4j_auth}'"
            exit 1
        fi
    fi
}
...
for i in $( set | grep ^NEO4J_ | awk -F'=' '{print $1}' | sort -rn ); do
    ...
    if [[ -n ${value} ]]; then
        if [[ ! "${setting}" =~ ^[0-9]+.*$ ]]; then
            ...
            echo "WARNING: ${setting} not written to conf file because settings that start with a number are not permitted"
        fi
    fi
done
...
if [ "${cmd}" == "dump-config" ]; then
    if [ ! -d "/conf" ]; then
        echo "You must mount a folder to /conf so that the configuration file(s) can be dumped to there."
        exit 1
    fi
...
if [ "${cmd}" == "neo4j" ]; then
    ...
    if [ -f "${NEO4J_HOME}/run/neo4j.pid" ]; then
        echo "PID file exists. Removing NEO4J_HOME/run/"
        rm -rf "${NEO4J_HOME}/run/"
    fi
